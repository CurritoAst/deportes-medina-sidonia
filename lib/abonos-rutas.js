...
async function usuarioCompleto(id) {
  ...
  if (!u) return null;
  const [a, c] = await Promise.all([abonos.abonoDe(id), abonos.carnetDe(id)]);
  return Object.assign(vistaUsuarioAdmin(u), { abono: abonos.vistaAbonoAdmin(a, c) });
}
...
function montar(router, { difundirAcceso } = {}) {
  ...
  router.ruta('GET', '/api/admin/usuarios', A, { handler: async (ctx) => {
    ...
    const lista = filas.slice(0, porPagina).map((f) => Object.assign(vistaUsuarioAdmin(f), {
      abono: abonos.vistaAbonoAdmin(f.a_hasta ? { activo: f.a_activo, desde: f.a_desde, hasta: f.a_hasta, auto_renovar: f.a_auto } : null,
  ...
  router.ruta('POST', '/api/admin/torno/validar', A, { esquema: { lectura: { tipo: 'texto', largoMax: 64 }, direccion: { tipo: 'enum', valores: ['entrada', 'salida'], opcional: true, porDefecto: 'entrada' } }, handler: async (ctx) => {
    const v = ctx.cuerpo.lectura.trim();
    ...
    if (s) {
      ...
      else { const e = edad(s.birthdate); const min = Number(await abonos.ajuste('edad_minima', 16)) || 16; if (e !== null && e < min) motivo = `Acceso a partir de ${min} años`; else { resultado = 'ok'; motivo = 'Abono en vigor'; } }
    }
    const r = await tornoRutas.registrarAcceso({ ts: Date.now(), metodo, resultado, motivo, direccion: dir, raw: v }, { origen: 'panel', dispositivo: 'panel' });
    if (!r.duplicado && difundirAcceso) difundirAcceso(r.evento);
    responder(ctx.res, 200, { resultado, motivo, direccion: dir, metodo, usuario: s ? { id: s.id, nombre: s.nombre } : null });
  } });
...
/* Amplía la vista del propio usuario con su abono (para /api/auth/yo). */
async function extrasUsuario(usuarioId) {
  const [a, c] = await Promise.all([abonos.abonoDe(usuarioId), abonos.carnetDe(usuarioId)]);
  return { abono: abonos.vistaAbonoPropio(a, c) };
}
