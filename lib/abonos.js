...
async function altaAbono({ usuarioId, meses, autoRenovar, nfcUid, actorId, ip }) {
  ...
  return bd.enTransaccion(async (con) => {
    ...
    // Pago en recepción: el alta se cobra con TARJETA en el datáfono; las
    // renovaciones/reactivaciones en mostrador. Queda el recibo ya PAGADO.
    await recibos.crear(con, {
      usuarioId, abonoId: a.id, periodo: d,
      concepto: tipo === 'alta' ? `Alta del abono (${meses} mes/es, tarjeta en recepción)` : `${tipo === 'renovacion' ? 'Renovación' : 'Reactivación'} en recepción (${meses} mes/es)`,
      importe: precio * meses, metodo: tipo === 'alta' ? 'tarjeta' : 'mostrador', estado: 'pagado', actorId
    });
...
/* Asigna el UID real de la pulsera (UNIQUE → 409 con el nombre de quien lo tiene).
   Acepta lo que escriba un lector USB de escritorio (decimal, hex, con
   separadores) y GUARDA SIEMPRE el decimal que entrega el lector del torno. */
async function asignarUid(con, usuarioId, nfcUid) {
  const n = uidMod.normalizar(nfcUid);
  if (!n.ok) return { error: n.error, codigo: 'UID_INVALIDO' };
  const uid = n.principal;
  ...
  // Choque contra la forma principal Y las alternativas (misma pulsera leída en otro orden de bytes)
  const formas = [uid].concat(n.alternativas);
  const [otro] = await q('SELECT c.usuario_id, u.nombre, c.nfc_uid FROM carnets c JOIN usuarios u ON u.id = c.usuario_id WHERE c.nfc_uid IN (?) AND c.usuario_id <> ?', [formas, usuarioId]);
  ...
  return { ok: true, nfcUid: uid, formato: n.formato, alternativas: n.alternativas, aviso: n.aviso };
}
...
async function sociosParaTorno() {
  const margen = await recibos.margenDias();
  const filas = await bd.consulta(
    `SELECT u.id, u.nombre, u.fecha_nacimiento, a.activo, a.hasta, c.nfc_uid, c.nfc_id_legacy, c.qr_uid, c.qr_seed,
            ...
            (SELECT MIN(r.devuelto_en) FROM recibos r WHERE r.usuario_id = u.id AND r.estado = 'devuelto') AS impago_desde
       FROM usuarios u
  ...
  return filas.map((f) => {
    ...
    return {
      ...
      gym,
      // Recibo devuelto: dentro del plazo la Pi AVISA; pasado `vence`, DENIEGA
      impago: f.impago_desde ? { desde: new Date(f.impago_desde).getTime(), vence: recibos.venceDe(f.impago_desde, margen) } : null
    };
  });
}
...
async function etagTorno() {
  const f = await bd.uno(
    `SELECT (SELECT COUNT(*) FROM carnets) c1, (SELECT COALESCE(MAX(UNIX_TIMESTAMP(emitido_en)),0) FROM carnets) c2,
            ...
            (SELECT COALESCE(MAX(UNIX_TIMESTAMP(actualizado_en)),0) FROM recibos) r1, (SELECT COUNT(*) FROM recibos WHERE estado = 'devuelto') r2,
            (SELECT COALESCE(MAX(UNIX_TIMESTAMP(actualizado_en)),0) FROM ajustes) j1`);
