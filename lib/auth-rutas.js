...
async function bootstrapAdmin(log) {
  ...
  // Si el correo YA existe (p. ej. se registró antes como vecino), se PROMOCIONA a
  // admin con esta contraseña en vez de fallar por el UNIQUE del email.
  const existente = await bd.uno('SELECT id, rol FROM usuarios WHERE email = ?', [email]);
  let id;
  if (existente) {
    await bd.consulta("UPDATE usuarios SET rol = 'admin', clave_hash = ?, clave_cambiada_en = ?, debe_cambiar_clave = 1, email_verificado_en = COALESCE(email_verificado_en, ?), eliminado_en = NULL, fallos_login = 0, bloqueado_hasta = NULL WHERE id = ?",
      [hash, ahora(), ahora(), existente.id]);
    await bd.consulta('DELETE FROM sesiones WHERE usuario_id = ?', [existente.id]);
    id = existente.id;
    await registrar('sistema', `Cuenta existente promocionada a admin por bootstrap: ${email} (era ${existente.rol}; debe cambiar la contraseña)`, null, null, 'usuario', id);
    log(`[bootstrap] ADMIN PROMOCIONADO: ${email} (ya existía como ${existente.rol}). Entra con la clave del bootstrap, cámbiala y BORRA las variables MSD_BOOTSTRAP_ADMIN_*.`);
  } else {
    ...
    id = r.insertId;
    await registrar('sistema', `Admin inicial creado por bootstrap: ${email} (debe cambiar la contraseña)`, null, null, 'usuario', id);
