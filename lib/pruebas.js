...
async function sembrar(ctx, actorId, ip) {
  ...
  // 1b) La primera persona de prueba recibe una contraseña ALEATORIA nueva en cada
  //     sembrado (se enseña una vez en el panel) para poder entrar como vecino normal.
  const clavePrueba = 'Prueba-' + crypto.randomBytes(5).toString('hex');
  await bd.consulta('UPDATE usuarios SET clave_hash = ?, clave_cambiada_en = NOW(3), debe_cambiar_clave = 0, fallos_login = 0, bloqueado_hasta = NULL WHERE id = ?',
    [await clave.hashear(clavePrueba), ids[0].id]);
  resumen.acceso = { email: `prueba1${DOMINIO}`, clave: clavePrueba, nombre: ids[0].nombre };
